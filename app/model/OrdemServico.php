...
class OrdemServico extends TRecord
{
    ...
    public function __construct($id = NULL, $callObjectLoad = TRUE)
    {
        ...
        parent::addAttribute('descricao_equipamento');
    ...
    public function get_conta_pedido_venda_to_string()
    {
        ...
        $values = Conta::where('ordem_servico_id', '=', $this->id)->getIndexedArray('pedido_venda_id','{pedido_venda->id}');
        return implode(', ', $values);
    }
  ...
  public function get_status()
    {
        if(date('Y-m-d') > $this->data_prevista && !$this->data_fim)
        {
            return "<label style='width:120px;' class='label label-danger'> EM ATRASO </label>";
        }
        elseif(!$this->data_fim )
        {
            return "<label style='width:120px;' class='label label-warning'> EM ABERTO </label>";
        }
        elseif($this->data_fim )
        {
            return "<label style='width:120px;' class='label label-success'> FINALIZADO </label>";
        }
    }
