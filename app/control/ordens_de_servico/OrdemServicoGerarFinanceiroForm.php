...
class OrdemServicoGerarFinanceiroForm extends TPage
{
    ...
    public function __construct( $param )
    {
        ...
        $conta_ordem_servico_categoria_id = new TDBCombo('conta_ordem_servico_categoria_id[]', 'minierp', 'Categoria', 'id', '{nome}','nome asc'  );
        $conta_ordem_servico_forma_pagamento_id = new TDBCombo('conta_ordem_servico_forma_pagamento_id[]', 'minierp', 'FormaPagamento', 'id', '{nome}','nome asc'  );
        $conta_ordem_servico_dt_vencimento = new TDate('conta_ordem_servico_dt_vencimento[]');
        ...
        $this->fieldlist_contas->addField(new TLabel("Data de vencimento", null, '14px', null), $conta_ordem_servico_dt_vencimento, ['width' => '20%']);
        ...
        $this->form->addField($conta_ordem_servico_dt_vencimento);
        ...
        $categoria_id->addValidation("Categoria da conta", new TRequiredValidator()); 
        ...
        $conta_ordem_servico_dt_vencimento->setMask('dd/mm/yyyy');

        $primeiro_vencimento->setDatabaseMask('yyyy-mm-dd');
        $conta_ordem_servico_dt_vencimento->setDatabaseMask('yyyy-mm-dd');
        ...
        $conta_ordem_servico_dt_vencimento->setSize(110);
        $conta_ordem_servico_categoria_id->setSize('100%');
        $conta_ordem_servico_forma_pagamento_id->setSize('100%');


        $row1 = $this->form->addFields([new TLabel("Código da OS:", null, '14px', null, '100%'),$id],[new TLabel("Cliente:", '#000000', '14px', null, '100%'),$cliente_id]);
        $row1->layout = [' col-sm-3',' col-sm-8'];

        $row2 = $this->form->addFields([new TLabel("Valor total:", null, '14px', null, '100%'),$valor_total],[new TLabel("Categoria da conta:", null, '14px', null, '100%'),$categoria_id],[new TLabel("Forma de pagamento:", null, '14px', null),$forma_pagamento_id]);
        $row2->layout = [' col-sm-3',' col-sm-4',' col-sm-4'];
    ...
    public static function onGerarParcelas($param = null) 
    {
        try 
        {
            ...
            (new TRequiredValidator)->validate('Identificação pessoa', $param['cliente_id']);
            ...
            $data->conta_ordem_servico_dt_vencimento = [];
            ...
            for($i = 0 ; $i < $parcelas; $i++)
            {
                ...
                $data->conta_ordem_servico_dt_vencimento[] = $data_vencimento->format('d/m/Y');
    ...
    public function onSave($param = null) 
    {
        try
        {
            ...
            //pegar dados de fora do form, para completar o objeto
            ...
            $detailObject->obs = "Titulo gerado automaticamente REF. ORDEM DE SERVIÇO = {$detailObject->ordem_servico_id}";
