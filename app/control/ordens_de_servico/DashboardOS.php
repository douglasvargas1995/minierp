...
class DashboardOS extends TPage
{
    ...
    public function __construct( $param = null)
    {
        ...
        $total_vendas_mes->setDatabase('minierp');
        ...
        $total_vendas_mes->setTarget(100000, '#ffffff', function($percentage, $target){
        ...
        $total_vendido_ano->setFieldValue("ordem_servico.valor_total");
        $total_vendido_ano->setModel('OrdemServico');
        ...
        $total_vendido_ano->setTarget(100000, '#ffffff', function($percentage, $target){
            return "{$percentage}% de R$ 100.000,00";
        });
        $total_vendido_ano->setColors('#2980B9', '#FFFFFF', '#3498DB', '#FFFFFF');
        $total_vendido_ano->setTitle("TOTAL VENDIDO NO ANO", '#FFFFFF', '20', '');
        $total_vendido_ano->setCriteria($criteria_total_vendido_ano);
        $total_vendido_ano->setIcon(new TImage('fas:shopping-basket #FFFFFF'));
        $total_vendido_ano->setValueSize("20");
        $total_vendido_ano->setValueColor("#FFFFFF", 'B');
        $total_vendido_ano->setSize('100%', 95);
        $total_vendido_ano->setLayout('horizontal', 'left');

        $total_vendas_por_mes->setDatabase('minierp');
        ...
        $total_por_mercadoria->setDatabase('minierp');
        ...
        $total_por_mercadoria->setTitle("TOTAL POR PRODUTO");
        ...
        $total_por_servico->setDatabase('minierp');
        ...
        $row1 = $this->form->addFields([new TLabel("Mês:", null, '14px', null, '100%'),$mes],[new TLabel("Ano:", null, '14px', null, '100%'),$ano],[new TLabel("Situação:", null, '14px', null, '100%'),$situacao],[new TLabel(" ", null, '16px', null, '100%'),$button_filtrar]);
        $row1->layout = [' col-sm-4',' col-sm-4',' col-sm-3',' col-sm-1'];

        $row2 = $this->form->addFields([$total_vendas_mes],[$total_vendido_ano]);
        ...
        if(!isset($param['situacao']) && $situacao->getValue())
        {
            $_POST['situacao'] = $situacao->getValue();
        }
        ...
        $filterVar = $searchData->situacao;
        if($filterVar)
        {
            $criteria_total_vendas_mes->add(new TFilter('ordem_servico.situacao', '=', $filterVar)); 
        }
        $filterVar = $searchData->ano;
        if($filterVar)
        {
            $criteria_total_vendido_ano->add(new TFilter('ordem_servico.ano', '=', $filterVar)); 
        }
        $filterVar = $searchData->ano;
        if($filterVar)
        {
        ...
        if($filterVar)
        {
            $criteria_total_vendas_por_mes->add(new TFilter('ordem_servico.situacao', '=', $filterVar)); 
        }
        $filterVar = $searchData->situacao;
        if($filterVar)
        {
        ...
        if($filterVar)
        {
            $criteria_total_por_mercadoria->add(new TFilter('ordem_servico_item.ordem_servico_id', 'in', "(SELECT id FROM ordem_servico WHERE situacao = '$filterVar')")); 
        }
        $filterVar = $searchData->mes;
        if($filterVar)
        {
        ...
        if($filterVar)
        {
            $criteria_total_por_servico->add(new TFilter('ordem_servico_item.ordem_servico_id', 'in', "(SELECT id FROM ordem_servico WHERE situacao = '$filterVar')")); 
        }

        BChart::generate($total_vendas_mes, $total_vendido_ano, $total_vendas_por_mes, $total_por_mercadoria, $total_por_servico);
